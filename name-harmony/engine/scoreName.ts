...
export function scoreName(
  first: string,
  last: string,
  nameMetadata?: any, // This parameter is used in `generateGivenNames` to pass the entry object
  ...
  const totalWeight = Object.values(weights).reduce((a, b) => a + b, 0);
    ...
    // Cultural Coherence - ACTUALLY USE the culture profiles
  ...
  // Distinctiveness
  const distinct = distinctiveness(first, US_ENGLISH_NAMES.map(n => n.given_name));
